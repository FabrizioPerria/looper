...
public:
    ...
    */
        ...
        , autoOpenMidiDevices (shouldAutoOpenMidiDevices)
    {
        ...
        if (preferredSetupOptions != nullptr) options.reset (new AudioDeviceManager::AudioDeviceSetup (*preferredSetupOptions));
    ...
    void showAudioSettingsDialog()
    {
        ...
        if (auto* bus = processor->getBus (true, 0)) maxNumInputs = jmax (0, bus->getDefaultLayout().size());

        if (auto* bus = processor->getBus (false, 0)) maxNumOutputs = jmax (0, bus->getDefaultLayout().size());
    ...
    {
#if JUCE_IOS
        if (auto device = dynamic_cast<iOSAudioIODevice*> (deviceManager.getCurrentAudioDevice())) device->switchApplication();
#endif
    }
    ...
    Image getIAAHostIcon ([[maybe_unused]] int size)
    {
        ...
        if (auto device = dynamic_cast<iOSAudioIODevice*> (deviceManager.getCurrentAudioDevice())) return device->getIcon (size);
#else
#endif
    ...
    static StandalonePluginHolder* getInstance() { return currentInstance; }
    ...
    public:
        ...
        void resized() override
        {
            ...
            if (owner.getProcessorHasPotentialFeedbackLoop())
            {
                ...
                shouldMuteButton.setBounds (Rectangle<int> (extra.proportionOfWidth (0.35f),
                                                            seperatorHeight,
...
public:
    ...
    StandaloneFilterWindow (const String& title, Colour backgroundColour, std::unique_ptr<StandalonePluginHolder> pluginHolderIn)
        ...
        , optionsButton ("Options")
    {
        ...
        const auto windowScreenBounds = [this]() -> Rectangle<int>
        {
            ...
            if (displays.displays.isEmpty()) return { width, height };
    ...
    */
    ...
    {
        if (button != nullptr && result != 0) button->handleMenuResult (result);
    }
...
private:
    ...
    void buttonClicked (Button* button) override
    {
        ...
        m.showMenuAsync (PopupMenu::Options().withTargetComponent (button), ModalCallbackFunction::forComponent (menuCallback, this));
    }
    ...
    public:
        ...
        {
            if (auto* e = editor.get()) return e->getConstrainer();
    ...
    private:
        ...
        void handleResized()
        {
            ...
            if (shouldShowNotification) notification.setBounds (r.removeFromTop (NotificationArea::height));
        ...
        void componentMovedOrResized (Component&, bool, bool) override { handleMovedOrResized(); }
